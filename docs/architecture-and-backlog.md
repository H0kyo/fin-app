...
- **Той самий flow (однакові `name`+`target`) не можна визначати у двох модулях `transformations/` одночасно** — навіть тимчасово "для діагностики". Стався реальний інцидент: `manual_transactions_flow` лишився продубльований і в `manual_transactions.py`, і в `united_transactions.py` (після діагностичного експерименту, що перевіряв, чи `create_streaming_table` + `append_flow` коректно лінкуються між файлами — коментар з поміткою "TEMPORARY DIAGNOSTIC... revert once diagnosed" не був прибраний). Результат — та сама помилка `No query found for dataset ... silver.transactions`: реєстрація графа для цього flow ламається, коли DLT завантажує обидва модулі й бачить дублікат. Правило: `united_transactions.py` містить **лише** `dp.create_streaming_table(name="silver.transactions", ...)`, без жодної `@dp.append_flow`/`@dp.table` функції; кожен flow живе рівно в одному файлі свого джерела (`manual_transactions.py`, згодом `mono_transactions.py`/`privat_transactions.py`). Якщо колись знадобиться діагностичне тимчасове переміщення коду між файлами — видаляти його одразу після діагностики, не лишаючи "TODO revert" у закомічених трансформаціях.
